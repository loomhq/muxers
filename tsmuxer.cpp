...
const unsigned kMaxBufferLength =
    2 * 4 * 3840 * 2160;  // twice the size of a raw ARGB 4K frame
...
class TsMuxerClass {
 public:
  ...
  u_char* muxH264(const u_char* data,
                  int data_len,
                  unsigned long pts_in_90khz,
                  int* muxed_len);
 ...
 private:
  ...
  bool shouldWritePsi(const uint8_t* data,
                      uint32_t length,
                      uint64_t pts_in_90khz,
                      Codec codec) const;
  void writePsiIfNecessary(const uint8_t* data,
                           uint32_t length,
                           uint64_t pts_in_90khz,
                           Codec codec);
...
const unsigned char* muxAac(struct TsMuxer* muxer,
                            ...
                            unsigned long pts_in_90khz,
                            int* muxed_len) {
  return (static_cast<TsMuxerClass*>(muxer->obj))
      ->muxAac(data, length, pts_in_90khz, muxed_len);
}
...
void TsMuxerClass::buildPmt() {
  ...
  pmt_.push_back(0xf0 | (program_info_length >>
  ...
  uint16_t section_length = static_cast<uint16_t>(pmt_.size()) -
                            section_length_last_pos +
                            4;  // 4 for the CRC not included yet
...
void TsMuxerClass::writeToBuffer(const u_char* data, size_t len) {
  ...
  if (static_cast<uint32_t>(buffer_.size()) < (bytes_written_ + len)) {
    buffer_.resize(static_cast<size_t>(bytes_written_) +
...
void TsMuxerClass::writeToBuffer(const std::vector<u_char>& data) {
...
bool TsMuxerClass::shouldWritePsi(const uint8_t* data,
                                  uint32_t length,
                                  uint64_t pts_in_90khz,
                                  Codec codec) const {
  ...
  if (codec == Codec::H264) {
    ...
    bool is_intra =
        H264Utils::hasAnnexBNalOfType(data, length,
...
void TsMuxerClass::writePsiIfNecessary(const uint8_t* data,
                                       uint32_t length,
                                       uint64_t pts_in_90khz,
                                       Codec codec) {
...
u_char* TsMuxerClass::muxAac(const u_char* data,
                             int data_len,
                             unsigned long pts_in_90khz,
                             int* muxed_len) {
  ...
  frame_to_encode.pts = pts_in_ts_timebase;
...
u_char* TsMuxerClass::muxH264(const u_char* data,
                              int data_len,
                              unsigned long pts_in_90khz,
                              int* muxed_len) {
  bytes_written_ = 0;  // Reset the buffer pointer
...
OutputStream* TsMuxerClass::getCurrentStream() const {
...
void TsMuxerClass::writePat() {
  std::array<u_char, 4> pat_header = {
      0x47, 0x40, 0x00, 0x10};  // 4 bytes  010 0000000000000 00 01 0000
  std::array<u_char, 13> pat_data_bytes = {0x00, 0x00, 0xb0, 0x0d, 0x00,
...
int TsMuxerClass::adaptationFieldLength() {
  OutputStream* stream = getCurrentStream();
  int pes_header_size = stream->pes_pid == PES_H264_PID ? PES_H264_HEADER_SIZE
                                                        : PES_ADTS_HEADER_SIZE;
  int afsize = 0;
...
void TsMuxerClass::write_adaptation_field_section() {
  ...
  if (packetHasPcr()) {
    field[1] |=
        0x50;  // Set PCR flag and random access indicator to 1 on I-frames
    // Write PCR
    field[2] = (pcr >> 25);
    field[3] = 0xff & (pcr >> 17);
    field[4] = 0xff & (pcr >> 9);
...
static void writePts(uint8_t* q, uint8_t four_bits, int64_t pts) {
  int val;
  val = four_bits << 4 | (((pts >> 30) & 0x07) << 1) | 1;
  *q++ = val;
...
void TsMuxerClass::writePesHeader(int adapfield_size) {
  ...
  pes_header.resize(getCurrentStream()->pes_pid == PES_ADTS_PID
                        ? PES_ADTS_HEADER_SIZE
                        : PES_H264_HEADER_SIZE);
  ...
  pes_header[6] =
      0x80;  // '10' (fixex) + '00' (scramble control) + '0' (priority) + '0'
             // (alignment) + '0' (copyright) + '0' (original)

  if (streams_.find(Codec::AAC) != streams_.end()) {
    // The size field will finish after byte 5
    aac_size =
        streams_.at(Codec::AAC).frame_size_bytes + (PES_ADTS_HEADER_SIZE - 6);
  }
...
int TsMuxerClass::writePesPayload() {
  OutputStream* stream = getCurrentStream();
  int max_possible_size_to_write =
      MPEGTS_PACKET_SIZE - (this->bytes_written_ % MPEGTS_PACKET_SIZE);
  unsigned bytes_to_write =
      std::min(max_possible_size_to_write, stream->frame_size_bytes);
...
void TsMuxerClass::writeH264AudIfAbsent() {
  ...
  if (nalu_code != kAudCode) {
    const u_char aud[6] = {0x00, 0x00, 0x00,
                           0x01, 0x09, 0xf0};  // f = all pic types + stop bit;
    writeToBuffer(aud, 6);
  }
}
